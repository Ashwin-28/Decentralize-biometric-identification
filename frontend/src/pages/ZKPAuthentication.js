...
const ZKPAuthentication = () => {
    ...
    const handleCapture = async () => {
        ...
        try {
            processedImage = await cropBiometricImage(imageSrc, biometricType, eyeSide);
        } catch (err) {
        ...
        try {
            ...
            setVerificationResult(result.error
                ...
                : { success: result.authenticated, message: result.message,
                    proof: result.proof, verification: result.verification,
                    similarity: result.similarity, privacy: result.privacy,
                    subjectName: result.subject_name });
        } catch (err) {
            setIsProcessing(false);
            setStep(4);
            setVerificationResult({ success: false, message: `Error: ${err.message}` });
        }
    };
    ...
    const handleSensorCapture = async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`${API_BASE}/fingerprint/capture`, { method: 'POST' });
            const data = await res.json();
            if (!data.success || !data.image_b64) {
                setError(data.error || 'Sensor capture failed. Ensure Windows Biometric Service is running.');
            ...
            setVerificationResult(result.error
                ? { success: false, message: result.error }
                : { success: result.authenticated, message: result.message,
                    proof: result.proof, similarity: result.similarity,
                    privacy: result.privacy, subjectName: result.subject_name });
        } catch (err) {
            setIsSensorCapturing(false);
            setStep(4);
            setVerificationResult({ success: false, message: `Error: ${err.message}` });
        ...
        <div className="zkp-container">
            ...
            <div className="zkp-card glass">
                ...
                {step === 1 && (
                    <div className="step-content animate-fade-in">
                        ...
                        <p className="description">Provide the identity identifier you registered with.</p>
                        ...
                        {isFingerprint ? (
                            <>
                                ...
                                <div className="sensor-box">
                                    <div className={`sensor-ring-zkp ${isSensorCapturing ? 'scanning' : ''}`}>
                                        <span className="sensor-icon-zkp">👆</span>
                                        {isSensorCapturing && <div className="scan-line-zkp" />}
                                    </div>
                                    <p className="sensor-status">
                                        {isSensorCapturing ? 'Scanning… Hold still' : 'Ready to scan'}
                                    </p>
                                </div>
                                ...
                                >
                                    {isSensorCapturing ? '⏳ Scanning…' : '👆 Scan & Generate ZKP'}
                                </button>
                            </>
                        ) : (
                            /* Face / Iris: webcam */
                                    ...
                                    <div className="webcam-overlay">
                                        {isIris ? (
                                            <div className={`iris-guide-zkp iris-guide-${eyeSide}`}>
                                                <div className="iris-label-zkp">{eyeSide.toUpperCase()} EYE</div>
                                            </div>
                                        ) : (
                ...
                {step === 3 && (
                    <div className="step-content animate-fade-in">
                        ...
                        <div className="progress-container">
                            <div className="progress-bar" style={{ width: `${proofProgress}%` }}></div>
                        </div>
                        ...
                        <div className="proof-steps">
                            {[
                                ...
                                <div key={label} className={`proof-step ${proofProgress > threshold ? 'done' : ''}`}>
                                    {label}
                                </div>
                            ))}
                        </div>
                ...
                {step === 4 && (
                    <div className="step-content animate-fade-in">
                        {!verificationResult ? (
                            ...
                            <div className={`result-container ${verificationResult.success ? 'success' : 'failure'}`}>
                                ...
                                {verificationResult.success && verificationResult.proof && (
                                    <div className="proof-details glass">
                                        ...
                                        <div className="detail-row">
                                            <span>Protocol:</span>
                                            <span>{verificationResult.proof.protocol} (zk-SNARK)</span>
                                        </div>
                                        <div className="detail-row">
                                            <span>Curve:</span>
                                        ...
                                        <div className="detail-row">
                                            ...
                                            <span>{(verificationResult.similarity * 100).toFixed(1)}%</span>
                                        </div>
                                        <div className="detail-row">
                                        ...
                                        {verificationResult.privacy && (
                                            <>
                                                <div className="detail-row">
                                                    ...
                                                    <span className="secure">
                                                        {verificationResult.privacy.biometric_revealed ? 'YES' : 'NO (Private)'}
                                                    </span>
                                                </div>
                                                <div className="detail-row">
                                                    <span>Proof Size:</span>
                                                    <span>{verificationResult.privacy.proof_size_bytes} bytes</span>
                                                </div>
                                            </>
                                        )}
                                    </div>
            ...
            <div className="zkp-info">
                <div className="info-card glass">
                    ...
                    <p>Your biometric data never leaves your device. Only a mathematical proof is sent to the server.</p>
                </div>
                <div className="info-card glass">
                    <h4>Blockchain Verified</h4>
                    <p>The proof is verified against the Poseidon hash commitment stored on-chain.</p>
                </div>
                <div className="info-card glass">
                    <h4>Groth16 Protocol</h4>
                    <p>Uses industry-standard zk-SNARK with bn128 elliptic curve for efficient verification.</p>
                </div>
            </div>
        </div>
    );
