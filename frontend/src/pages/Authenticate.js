...
import { authenticateSubject, captureFingerprint } from '../services/api';
...
function Authenticate() {
  ...
  const captureSensor = async () => {
    ...
    const onCaptureSuccess = (data) => {
      ...
      if (typeof setCapturedImage !== 'function') {
        console.error('[FATAL] State setter "setCapturedImage" is not a function!');
        return;
      }
      ...
      if (data.hash) {
        ...
        console.log('[AUTH] Fingerprint hash stored for verification:', data.hash.substring(0, 20) + '...');
      }
      
      if (data.simulated) {
        setSensorStatus('⚠️ Sensor unavailable — simulated fingerprint captured.');
      } else {
    ...
    const runCaptureWithRetry = async (retryCount = 0) => {
      const MAX_RETRIES = 2;
      setSensorStatus('Sensor is ready. Please place your finger on the sensor.');
      ...
      try {
        ...
        const keepAlive = setInterval(() => {
          console.debug(`[HEARTBEAT] Biometric bridge active (Scan ${retryCount + 1})`);
        }, 3000);
  ...
  const verifyUserByName = async () => {
    ...
    try {
      ...
      if (data.exists) {
        ...
        setError('User not found in dashboard.');
        setIsUserVerified(false);
      }
    } catch (err) {
      setError('Connection error while checking user.');
    } finally {
  ...
  const handleAuthenticate = async () => {
    ...
    try {
      ...
      const errMsg = err.response?.data?.error || 'Verification encountered an error.';
      setError(errMsg);
    ...
    <div className="page auth-page">
      <div className="container">
        <div className="page-header text-center">
          ...
          <h1>Authenticate</h1>
          <p className="text-muted">Verify your identity against the blockchain record</p>
        </div>
            ...
            {isFingerprint && !capturedImage && (
              <div className="sensor-capture-container">
                <div className="sensor-visual">
                  <div className={`sensor-ring ${isSensorCapturing ? 'scanning' : ''}`}>
                    <div className="sensor-icon">👆</div>
                    {isSensorCapturing && <div className="sensor-scan-line" />}
                  </div>
                  <p className="sensor-label">
                    {isSensorCapturing ? (sensorStatus || 'Scanning… Hold still') : 'Ready (Touch sensor)'}
                  </p>
                </div>
                {error && <div className="error-message">{error}</div>}
                <div className="action-buttons centered">
                  <button className="btn btn-primary" onClick={captureSensor} disabled={isSensorCapturing}>
                    {isSensorCapturing ? '⏳ Scanning…' : '👆 SCAN FINGERPRINT'}
                  </button>
                  <button className="btn btn-outline" onClick={reconnectSensor} disabled={isSensorCapturing}>
                    {isSensorCapturing ? '⏳ Resetting...' : '🔄 RECONNECT'}
                  </button>
                </div>
                <div className="step-guide mt-md">
                  <p><strong>Step 1:</strong> Place finger on sensor and click scan</p>
                </div>
              </div>
            )}
                  ...
                  <div className="voice-capture-container">
                    {!isUserVerified ? (
                      <div className="voice-lookup-step">
                        <p className="capture-instruction">Step 1: Verify your identity in the system</p>
                        <input
                          type="text"
                          className="input mb-md"
                          value={searchName}
                          onChange={(e) => setSearchName(e.target.value)}
                          placeholder="Type your registered name (e.g. John Doe)"
                          autoComplete="off"
                        />
                        <button 
                      ...
                      <>
                        ...
                        <div className="challenge-display card-glass mb-md">
                          <span className="mono-label">Voice Password Verification</span>
                          <div className="challenge-code" style={{fontSize: '1.2rem'}}>🔐 Speak Your Enrollment Password</div>
                          <p className="text-muted small">Say the same password you used during registration</p>
                        </div>
                        ...
                        <div className="action-buttons centered">
                          {!isRecording ? (
                            ...
                            <button 
                              ...
                              title={recordingTime < 2 ? 'Speak for at least 2 more seconds' : 'Stop recording'}
                            >
                              ⏹ {recordingTime < 2 ? `Speak... (${2 - recordingTime}s)` : 'Stop & Save'}
                            </button>
                          )}
                        </div>
                      </>
              ...
              <div className="captured-container">
                {isVoice ? (
                  ...
                  <div className="fingerprint-preview-container">
                    ...
                    {isFingerprint && (
                      <div className="step-guide mt-md">
                        <p className="text-emerald"><strong>Step 2:</strong> Fingerprint captured. Click VERIFY to continue.</p>
                      </div>
                    )}
                  </div>
                )}
                <div className="capture-actions">
                  <button className="btn btn-outline" onClick={reset}>↺ Retake</button>
                  <button
                    className="btn btn-primary"
                    id="verify-button"
          ...
          <div className="auth-result card">
            ...
            {result && !error && (
              <div className={`result-display ${result.success ? 'success' : 'failure'}`}>
                ...
                <h4>{result.success ? 'Verification Successful' : 'Verification Failed'}</h4>
                <p style={{ 
                  fontSize: '1.2rem', 
                  fontWeight: 'bold', 
                  color: result.success ? 'var(--accent-emerald)' : 'var(--accent-ruby)',
                  margin: '1rem 0'
                }}>
                  {result.ui_message}
                </p>
                <p>{result.message || (result.success
                  ? 'Your identity has been verified successfully.'
                  : 'Biometric verification failed. Please try again.')}</p>
                ...
                {result.hashes && (
                  <div className="result-meta" style={{ marginTop: '1rem' }}>
                    <div style={{ marginBottom: '0.5rem', fontSize: '0.85rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>
                      Cryptographic Proof
                    </div>
                    <div className="meta-item">
